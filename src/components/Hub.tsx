...
function renderMd(md: string): string {
  return md
    ...
    .replace(/^(?!<[hl]|<li)(.+)$/gm, (m: string) => m.trim() ? `<p class="text-stone-400 text-sm leading-relaxed mb-2">${m}</p>` : '');
}
...
function AboutSection({ location, education, hobbies, aboutMd }: Pick<Props, 'location' | 'education' | 'hobbies' | 'aboutMd'>) {
...
function ProjectsSection({ projects, github }: { projects: Props['projects']; github: string }) {
  return (
    <div className="h-full overflow-y-auto animate-fadeIn">
...
function YouTubeSection({ apiKey, channelId }: { apiKey: string; channelId: string }) {
  return (
    <div className="h-full overflow-y-auto animate-fadeIn">
...
function ContactSection({ email, github, linkedin }: Pick<Props, 'email' | 'github' | 'linkedin'>) {
  return (
