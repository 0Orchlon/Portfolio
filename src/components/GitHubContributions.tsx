    ...
    <div className="flex flex-col gap-1">
      <div className="flex gap-px mb-1" style={{ paddingLeft: '0px' }}>
        {weeks.map((_, wi) => {
          ...
          return (
            <div key={wi} className="w-3 text-[8px] text-stone-600 leading-none">
              {label ? label.month : ''}
            </div>
          );
        })}
...
function AccountGraph({ username }: { username: string }) {
  ...
  if (loading) {
    return <div className="flex items-center justify-center py-4 text-stone-600 text-sm">Loading...</div>;
  }

  if (days.length === 0) {
    return <div className="flex items-center justify-center py-4 text-stone-500 text-sm">No data</div>;
  }
...
export default function GitHubContributions({ users }: { users: string[] }) {
  ...
  return (
    <div className="p-4 bg-stone-800/30 rounded-lg border border-stone-700/30">
      <h3 className="text-sm font-semibold text-amber-100 mb-3">GitHub Activity</h3>
